...
class IMAGENETDataset(Dataset):
    ...
    def __init__(
        ...
        self.imagenete = torchvision.datasets.Imagenette(
            root=root,
            split=split,
            size=size,
            download=download,
    ...
    def __len__(self):
        return len(self.imagenete)

    def __getitem__(self, idx):
        image, label = self.imagenete[idx]
...
def show_one_per_class(dataset: Any, dir_path: str):
    """
    ...
    if len(class_samples) != 10:
        print(f"Предупреждение: найдено  {len(class_samples)} классов из 10")
    ...
    cv2.imwrite(f'{dir_path}/imagenete.png', grid)


if __name__ == '__main__':
    root_path = Path(__file__).parent.parent.parent.parent

    parser = argparse.ArgumentParser(description='Get PyTorch ImageNet dataset')
    parser.add_argument('--path', type=str, default='data/datasets/imagenet', help='dataset path')
    ...
    train_dataset = IMAGENETDataset(dataset_path, split='train', download=download_flag)
    test_dataset = IMAGENETDataset(dataset_path, split='val', download=download_flag)
